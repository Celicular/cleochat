...
async function checkSession(){
    ...
    if(cookie){
        ...
        document.querySelector(".invID").textContent = `My Invite ID: ${data.myInviteId}`;
        console.log(data.myInviteId);

        loadMessages(localStorage.getItem("addressUser"), localStorage.getItem("currentChatRecieverAddress"));
...
document.querySelector(".closeContactDialog").addEventListener('click', (e) => {
    console.log("closed");
...
messages.forEach((msg, i) => {
    if(i === 0){
        ...
        if(prev.classList.contains("recieve") && msg.classList.contains("recieve")){
            msg.classList.add("receivecollapse");
            prev.style.borderBottomLeftRadius = "0";
        }else if(prev.classList.contains("sent") && msg.classList.contains("sent")){
...
function setdata(username){
    ...
    .then(data => {
        ...
        document.querySelector(".chatnamedata>h3").textContent = localStorage.getItem("currentChatName").split(" ")[0];
...
document.querySelector(".send").addEventListener("click", (e) => {
    sendMessage(localStorage.getItem("addressUser"), localStorage.getItem("currentChatRecieverAddress"))
})
